...
import { Alert, Button, Descriptions, Empty, Form, Input, Modal, Select, Skeleton, Space, Tag, Typography, Upload, message } from 'antd';
...
import { useCreateYeuCauCapNhat, useDanhSachHoSoNhanSu, useHoSoCaNhan, useUploadFileHoSo, useUploadFileHopDong } from './api';
...
const HO_SO_TYPES = [
  {
    value: 'thoi_viec',
    label: 'Quyết định thôi việc/Chấm dứt Hợp đồng',
    description: 'Lưu hồ sơ khi nhân viên nghỉ việc, gồm biên bản bàn giao công việc và tài sản.',
  },
  {
    value: 'dieu_chuyen',
    label: 'Quyết định điều chuyển',
    description: 'Ghi nhận thay đổi vị trí làm việc, luân chuyển bộ phận hoặc dự án.',
  },
  {
    value: 'khen_thuong_ky_luat',
    label: 'Quyết định khen thưởng/Kỷ luật',
    description: 'Lưu lịch sử tuân thủ và đóng góp để phục vụ đánh giá cuối năm.',
  },
  {
    value: 'nang_luong',
    label: 'Quyết định nâng lương/Điều chỉnh bậc lương',
    description: 'Ghi nhận thay đổi đãi ngộ để phục vụ đối soát lương.',
  },
];
...
export default function HoSoNhanSuPage() {
  ...
  const uploadProfileFilesMutation = useUploadFileHoSo();
  const [isUpdateModalVisible, setIsUpdateModalVisible] = useState(false);
  const [selectedHoSoType, setSelectedHoSoType] = useState(HO_SO_TYPES[0].value);
            ...
            <>
              ...
              <div className="hicas-card p-5">
                <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <h3 className="m-0 flex items-center gap-2 text-lg font-semibold text-hicas-text">
                      <FolderOpenOutlined className="text-hicas-primary" />
                      Hồ sơ/Quyết định
                    </h3>
                    <p className="m-0 mt-1 text-sm text-hicas-muted">
                      Lưu trữ quyết định thôi việc, điều chuyển, khen thưởng/kỷ luật và nâng lương của nhân viên.
                    </p>
                  </div>
                  {canUploadContract && (
                    <Space wrap>
                      <Select
                        ...
                        value={selectedHoSoType}
                        options={HO_SO_TYPES.map((item) => ({ label: item.label, value: item.value }))}
                        onChange={setSelectedHoSoType}
                      />
                      <Upload
                        multiple
                        ...
                        beforeUpload={(file, fileList) => {
                          ...
                          uploadProfileFilesMutation.mutate(
                            { idNhanVien: selectedId, loaiHoSo: selectedHoSoType, files: fileList as File[] },
                            {
                              onSuccess: () => {
                                message.success('Đã upload file hồ sơ');
                                profileQuery.refetch();
                              },
                              onError: (error) => {
                                message.error(getBackendMessage(error) || 'Upload file hồ sơ thất bại');
                              },
                            },
                          );
                      ...
                      >
                        <Button icon={<UploadOutlined />} loading={uploadProfileFilesMutation.isPending}>
                          Upload hồ sơ
                        </Button>
                      </Upload>
                    </Space>
                ...
                <div className="grid gap-4 xl:grid-cols-2">
                  {HO_SO_TYPES.map((type) => {
                    const files = (profileQuery.data?.ho_so_tai_lieu ?? []).filter((item) => item.loaiHoSo === type.value);
                    return (
                      <div key={type.value} className="rounded-lg border border-hicas-border bg-white p-4">
                        <div className="mb-3">
                          <div className="font-semibold text-hicas-text">{type.label}</div>
                          <Text className="text-sm text-hicas-muted">{type.description}</Text>
                        </div>
                        {files.length === 0 ? (
                          <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Chưa có file" />
                        ) : (
                          <div className="space-y-2">
                            {files.map((file) => (
                              <div key={file.id} className="flex flex-col gap-2 rounded-lg bg-[#f7f9fc] p-3 md:flex-row md:items-center md:justify-between">
                                <div className="min-w-0">
                                  <div className="flex items-center gap-2 font-medium text-hicas-text">
                                    <PaperClipOutlined className="text-hicas-primary" />
                                    <span className="truncate">{file.tenFile}</span>
                                  </div>
                                  <Text className="text-xs text-hicas-muted">
                                    {(file.kichThuoc / 1024).toFixed(1)} KB · {formatDate(file.thoiGianUpload)}
                                  </Text>
                                </div>
                                <Button icon={<LinkOutlined />} href={fileUrl(file.duongDan) ?? undefined} target="_blank" rel="noreferrer">
                                  Mở file
                                </Button>
                              </div>
                            ))}
