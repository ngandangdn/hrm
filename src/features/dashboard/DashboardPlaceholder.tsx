...
const statCards = [
  ...
  { icon: <PieChartOutlined />, label: 'Quỹ phép còn lại', value: '6.5', suffix: 'ngày', footer: 'Năm 2023' },
    ...
    <div className="space-y-8">
      <section className="rounded-[14px] bg-gradient-to-r from-[#3f2be8] to-[#6f63ee] px-10 py-10 text-white shadow-hicas">
        <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="m-0 text-[30px] font-bold">Chào buổi sáng, Nguyễn Văn A 👋</h1>
            <p className="mt-3 text-lg text-white/85">Hôm nay: Thứ Tư, 24 Tháng 10, 2023</p>
          </div>
      ...
      <section className="grid gap-5 lg:grid-cols-4">
        {statCards.map((card) => (
          <div key={card.label} className="hicas-card p-6">
            ...
            {card.label === 'Đơn chờ duyệt' && <Button className="mt-6 w-full">Duyệt ngay</Button>}
          </div>
        ))}
      </section>
      ...
      <section className="grid gap-6 lg:grid-cols-[1fr_400px]">
        <div className="space-y-6">
          <div className="grid gap-5 md:grid-cols-3">
            {[
              ['Tạo đơn phép', <CalendarOutlined />],
              ['Giải trình chấm công', <ClockCircleOutlined />],
              ['Đặt phòng họp', <FormOutlined />],
            ].map(([label, icon]) => (
              <button key={String(label)} className="hicas-card flex h-[136px] flex-col items-center justify-center gap-3 text-lg">
                <span className="hicas-icon-tile text-2xl">{icon}</span>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="hicas-card overflow-hidden">
            ...
            <div className="divide-y divide-hicas-border">
              {[
                ['HÔM NAY', '24', 'Ca làm việc hành chính', 'Họp giao ban Phòng Nhân sự (14:00 - 15:30)', 'Đang làm việc'],
                ['THỨ 5', '25', 'Ca làm việc hành chính', '08:00 - 17:30', ''],
                ['THỨ 2', '29', 'Nghỉ phép năm (Đã duyệt)', '', 'Đã duyệt'],
              ].map((row) => (
        ...
        <aside className="space-y-6">
          <div className="hicas-card p-8">
            ...
            <div className="mt-8 space-y-4 text-base">
              <div className="flex justify-between"><span>Thời gian làm việc</span><strong>18.5 ngày</strong></div>
              <div className="flex justify-between"><span>Nghỉ phép/Lễ</span><strong>1.5 ngày</strong></div>
              <div className="flex justify-between"><span>Vắng mặt/Nghỉ không lương</span><strong>0 ngày</strong></div>
            </div>
          </div>
          ...
          <div className="hicas-card overflow-hidden">
            ...
            <div className="p-6">
              ...
              <p className="text-hicas-muted">Đơn xin nghỉ ngày 29/10/2023 của bạn đã được Trưởng phòng duyệt.</p>
            </div>
          </div>
        </aside>
      </section>
